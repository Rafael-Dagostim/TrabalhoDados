...
from functions import cls, print_pretty_pokemon, sort_pokemon_by_name_asc, find_pokemon_by_id
...
def see_pokemon_menu():
  cls()
  order_by = int(input('1 - Ordenar por nome\n2 - Ordenar por ID\nOutro - Voltar\nEscolha uma opção: '))
  if order_by == 1:
    pokemon_list = sort_pokemon_by_name_asc(pokemon_data)
  elif order_by == 2:
...
def find_pokemon_menu():
  ...
  while value != 0:
    ...
    print_pretty_pokemon(find_pokemon_by_id(pokemon_data, pokemon_id))
...
def main_menu():
    ...
    while value != 0:
      ...
      if value == 1: see_pokemon_menu()
      if value == 2: find_pokemon_menu()
