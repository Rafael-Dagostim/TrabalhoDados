...
def binary_search_by_pokemon_name(name, pokemon_list):
    """
    Retorna o objeto de um Pokémon pesquisando com parâmetro o seu nome.
    Feito por Yuri Machado.
    """
    pokemon_list = sort_pokemon_by_name_asc(pokemon_list)
    # Posição inicializada com -1, pois não existe nenhum pokemon com o nome procurado até então.
    position_found = -1
    ...
    while position_found == -1:
        ...
        size_divider = round(len(pokemon_list)/(2**i))

        if len(pokemon_list)/(2**i) < 1:
            size_divider = 1
            add_odd = 1
        ...
        if pokemon_list[size]['name'] == name:
            position_found = size
        elif pokemon_list[size]['name'] > name:
            high_low = 0
        else:
            high_low = 1
